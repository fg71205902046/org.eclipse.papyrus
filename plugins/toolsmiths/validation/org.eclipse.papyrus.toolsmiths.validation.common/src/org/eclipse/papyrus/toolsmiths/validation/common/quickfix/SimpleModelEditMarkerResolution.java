...
public class SimpleModelEditMarkerResolution<T extends EObject> extends AbstractPapyrusWorkbenchMarkerResolution {
	...
	public void run(IMarker marker) {
		Try<EditingDomain> openDomain = CommonMarkerResolutionUtils.getOpenEditingDomain(marker);
		if (openDomain.isFailure()) {
			// User cancelled prompt to save the dirty editor
			return;
		}
		...
		try {
			...
			// resources in it, so any item-provider adapters will have been clean up, too
