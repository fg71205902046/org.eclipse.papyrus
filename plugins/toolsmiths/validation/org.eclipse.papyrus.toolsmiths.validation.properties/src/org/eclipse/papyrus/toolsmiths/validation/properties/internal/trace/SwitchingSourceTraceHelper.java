...
abstract class SwitchingSourceTraceHelper extends ComposedSwitch<EObject> implements SourceTraceHelper {
	...
	protected abstract EObject getOwningClass(EObject property);
	...
	protected class ContextsDelegate extends ContextsSwitch<EObject> {
		 ...
		 * these objects.
		 */
		private final Set<EObject> upwardRecursionSet = new HashSet<>();
		...
		public EObject caseDataContextElement(DataContextElement object) {
			...
			if (!object.getProperties().isEmpty()) {
				// Find the source property of any context-property that provides one, then get its class
				result = drillDown(object.getProperties(), SwitchingSourceTraceHelper.this::getOwningClass);
			}
			...
			if (result == null) {
				...
				if (object.eContainer() instanceof DataContextPackage && upwardRecursionSet.add(object)) {
					try {
						EObject sourcePackage = composedSwitch(object.eContainer());
						if (sourcePackage != null) {
							result = SwitchingSourceTraceHelper.this.getClass(sourcePackage, object);
						}
					} finally {
						upwardRecursionSet.remove(object);
					}
				}
			}
